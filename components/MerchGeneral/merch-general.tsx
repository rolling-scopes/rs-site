...
export const MerchGeneral: FC<MerchGeneralProps> = ({
    ...
    const getImagesList = (images) => images.map((image) => {
        const {caption} = image;
        return (<div
    ...
    const getImageBlock = (images) => <div className="grid flex-shrink-0 grid-cols-1 gap-y-3 lg:gap-y-4">
                            ...
                            >
                                <div
                                    ...
                                    <div className="flex items-center space-x-6 lg:space-x-8">
                                            ...
                                            {getImageBlock([imagesList[3], imagesList[2], imagesList[4]])}
                                            {getImageBlock([imagesList[5], imagesList[6], imagesList[7]])}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <Link href={link}
                              className="inline-block rounded-md border border-transparent bg-night-900 py-3 px-8 text-center font-medium text-white hover:bg-zinc-400">{titleLink}</Link>
